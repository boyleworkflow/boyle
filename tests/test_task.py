...
def test_by_default_inherits_parent_levels():
    ...
    assert derived_l1.levels == parent_l1.levels


def test_default_can_be_overridden():
    parent_l1 = TASK_L1
    other_levels = (Name("any"), Name("here"))
    derived_l1 = Task({"inp": parent_l1["out"]}, "op", ["out"], levels=other_levels)
    assert derived_l1.levels == other_levels
...
def test_descend_creates_level():
    assert TASK_L0.descend("level1").levels == (Name("level1"),)


def test_descend_twice_creates_two_levels():
    assert TASK_L0.descend("l1").descend("l2").levels == (Name("l1"), Name("l2"))
